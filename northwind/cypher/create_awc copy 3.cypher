...
SET n += row,
    n.LongDescription = row.LongDescription,
    ...
    n.ReorderPoint = row.ReorderPoint,
...
SET n += row,
    n.NAME = row.NAME,
    n.ContactName = row.ContactName,
    n.ContactTitle = row.ContactTitle,
    n.ADDR1 = row.ADDR1,
    n.City = row.City,
    n.State = row.State,
    n.Zip = row.Zip,
...
SET n += row,
    ...
    n.ContactPhone = row.ContactPhone,
    n.ContactFax = row.ContactFax;
...
MATCH (o:Order)-[:ORDERS]->(p:Product)-[:PART_OF]->(c:Category), (o)<-[:PURCHASED]-(cust:Customer)
MERGE (cust)-[:USES]->(c);

MATCH (n:Product)
RETURN n.LongDescription AS name, n;
